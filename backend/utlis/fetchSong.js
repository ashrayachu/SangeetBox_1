...
const fetchSong = (songUrl) => {
    return new Promise((resolve, reject) => {
        ...
        const options = {
            method: 'GET',
            hostname: 'spotify-music-mp3-downloader-api.p.rapidapi.com',
            port: null,
            path: `/download?link=${encodeURIComponent(songUrl)}`,
            headers: {
                'x-rapidapi-key':process.env.RAPIDAPI_KEY,
        ...
        const apiRequest = https.request(options, (apiResponse) => {
            ...
            apiResponse.on("end", () => {
                try {
                    const body = JSON.parse(Buffer.concat(chunks).toString());
                    console.log("body", body)
                    if (!body.success || !body.data.medias) {
                        const errorMessage = body.message || "Failed to get download link from API";
                        return reject(new Error(`API Error: ${errorMessage}`));
                    }

                    resolve({
                        audioUrl: body.data.medias[0].url,
                        songName: body.data.title,
